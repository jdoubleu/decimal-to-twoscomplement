...
(function (w, d, V) {
  ...
  * @see repr
  */
	var reprReverse = function reprReverse(s) {
		s = s.toString();
  ...
  * @returns {String}
  */
	var convertDecimalToBase = function convertDecimalToBase(number) {
  ...
  * @returns {number}
  */
	var convertBaseToDecimal = function convertBaseToDecimal(number) {
  ...
  * @param {string} bin
  */
	var invertBinary = function invertBinary(bin) {
		// Flip all bits
		var b = bin.split('').map(function (v) {
			return v == '1' ? '0' : '1';
		});
  ...
  * @throws RangeError
  */
	var twosComplement = function twosComplement(number) {
		...
		if (sign == '+' && res >= Math.pow(2, wordLength - 1) || res > Math.pow(2, wordLength - 1)) throw new RangeError('Given number can\'t be represented by current word bit length (' + wordLength + ').');
  ...
  */
	new V({
		...
		methods: {
			gettwcs: function gettwcs(value) {
				var tc = void 0,
				    res = {
					bin: invalidValue,
					oct: invalidValue,
				...
				var _convertAndFill = function _convertAndFill(base) {
					var res = convertDecimalToBase(convertBaseToDecimal(tc, 2), base);
				...
				try {
					...
					if (checkNumber(value) && tc.length <= wordLength) res = {
						bin: tc,
